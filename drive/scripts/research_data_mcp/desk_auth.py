...
def session_cookie_value(
    token: str, *, issued_at: int | None = None, nonce: str | None = None
) -> str:
    ...
    payload = f"{_SESSION_VERSION}.{issued}.{entropy}"
...
def desk_session_cookie_valid(handler: BaseHTTPRequestHandler, token: str) -> bool:
    ...
    if not got or not token:
        return False
    parts = got.split(".")
    if len(parts) != 4 or parts[0] != _SESSION_VERSION:
        # Reject every deterministic v1 cookie immediately on deployment.
        return False
    ...
    if age < -_CLOCK_SKEW_SECONDS or age > _session_max_age():
        return False
    payload = f"{version}.{issued}.{nonce}"
...
def request_has_desk_access(handler: BaseHTTPRequestHandler) -> bool:
    ...
    if request_presents_desk_token(handler):
        return True
    return desk_session_cookie_valid(handler, token)
...
def desk_capability_document(handler: BaseHTTPRequestHandler) -> dict[str, object]:
    ...
    configured = bool(access_token_required())
    authenticated = request_has_desk_access(handler)
    return {
        "version": 1,
        "authenticated": authenticated,
        "server_configured": configured,
        "access": "operator" if authenticated else "locked",
        "permissions": {
            "view_static_ui": True,
            "view_research_data": authenticated,
            "view_faculty_profile": authenticated,
            "view_operations": authenticated,
            "use_ask": authenticated,
            "submit_collection": authenticated,
            "approve_jobs": authenticated,
        },
        "session": {
...
def authorize(handler: BaseHTTPRequestHandler, path: str, method: str = "GET") -> tuple[bool, str]:
    ...
    if request_has_desk_access(handler):
        return True, ""
