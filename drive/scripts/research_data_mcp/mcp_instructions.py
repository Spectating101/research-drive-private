...
def mcp_server_instructions() -> str:
    if os.getenv("RESEARCH_MCP_DESK", "").strip() in {"1", "true", "yes"}:
        return (
            ...
            "Discover procurement ladder: research_discover_create_intent, then research_discover_propose_intent, then wait for or record explicit review with research_discover_review_intent, select exactly one reviewed route with research_discover_select_intent_route, and only then call research_discover_submit_intent. Submission creates pending_approval only; never replace this ladder with a direct generic job when a Discover candidate is being acquired. Public members may investigate and plan but cannot submit; members may submit; only operators may approve. "
    ...
    return (
        ...
        "Cursor webfetch remains model-owned: pass its fetched URL, selected URL, and link/hash evidence to research_webfetch_handoff; the backend never fetches or chooses a replacement. "
