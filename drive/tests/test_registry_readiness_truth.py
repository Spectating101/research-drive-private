...
def test_datacite_parquet_uses_supported_smoke_backend(tmp_path: Path):
    target = tmp_path / "data_lake/procured/datacite/data.parquet"
    target.parent.mkdir(parents=True)
    pd.DataFrame([{"id": 1}, {"id": 2}]).to_parquet(target, index=False)
    promoter = _promoter(tmp_path)

    entry = promoter.promote_datacite_collect(
        _datacite_job("data_lake/procured/datacite", "data.parquet"),
        doi="10.1234/example",
    )[0]

    assert entry["backend"] == "local_parquet_panel"
    assert entry["analysis_readiness"] == "query_ready"
    assert entry["query_smoke"]["ok"] is True
    assert entry["query_smoke"]["rows"] == 2
    assert entry["local_root"] == "data_lake/procured/datacite"
    assert entry["local_file"] == "data.parquet"
