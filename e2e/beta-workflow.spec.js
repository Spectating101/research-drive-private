...
test.describe("beta workflow @ live-desk", () => {
  test("1 — header shows live registry, not demo seed", async ({ page }) => {
    ...
    await expect(page.locator(".rd-v2-trust-badge.ok", { hasText: /^Live$/ })).toBeVisible({
      timeout: 45_000,
    });
    await expect(page.locator(".rd-v2-trust-badge", { hasText: /^Demo/ })).toHaveCount(0);
    await expect(page.locator(".rd-v2-header-meta-count", { hasText: /\d+\s*ds/ })).toBeVisible();
