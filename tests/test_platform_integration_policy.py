...
def integration_cfg() -> dict:
    if not INTEGRATION.is_file():
        pytest.skip("optional Alpha integration config is not present in this checkout")
...
@pytest.mark.skipif(not CYCLE.is_file(), reason="optional Alpha cycle is not present in this checkout")
