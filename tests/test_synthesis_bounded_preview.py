"""Bounded Synthesis Preview/Test contract.

These tests deliberately prove the authority boundary, not just response shape:
preview uses the production recipe semantics without materialising an asset, and
an execution approval job cannot exist until the same accepted spec revision has
a successful preview receipt.
"""
...
def test_preview_reuses_execution_semantics_without_materialising(tmp_path: Path):
    ...
    receipt = run_bounded_preview(root, spec, input_row_limit=5, output_row_limit=10)

    assert receipt["status"] == "succeeded"
    assert receipt["bounded"] is True
    assert receipt["sampling"]["source_rows"] == 12
    assert receipt["sampling"]["previewed_rows"] == 5
    assert receipt["sampling"]["source_truncated"] is True
    assert receipt["rows"]["preview_input"] == 5
    assert receipt["rows"]["output"] == 1
    assert receipt["output"]["columns"] == ["asset", "mean_value"]
    assert receipt["output"]["rows"][0]["asset"] == "A"
    assert receipt["output"]["rows"][0]["mean_value"] == 3.0
...
def test_first_submit_call_records_preview_and_creates_no_job(tmp_path: Path):
    gateway, store, thread, _spec = _accepted_gateway(tmp_path)

    out = gateway.synthesis_thread_submit_execution(thread["id"])
...
def test_second_submit_call_creates_pending_approval_for_previewed_revision(tmp_path: Path):
    gateway, store, thread, _spec = _accepted_gateway(tmp_path)

    preview = gateway.synthesis_thread_submit_execution(thread["id"])
    assert preview["preview_only"] is True
    submitted = gateway.synthesis_thread_submit_execution(thread["id"])
...
def test_stale_preview_never_authorizes_changed_spec(tmp_path: Path):
    from scripts.research_data_mcp.synthesis_preview import execution_spec_hash
    from scripts.research_data_mcp.synthesis_executor import validate_execution_spec

    gateway, store, thread, spec = _accepted_gateway(tmp_path)
    first = gateway.synthesis_thread_submit_execution(thread["id"])
