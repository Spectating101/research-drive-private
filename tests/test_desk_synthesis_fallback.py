...
def test_fallback_status_expires_after_runtime_ttl(monkeypatch):
    from scripts.research_data_mcp import desk_synthesis_fallback as fallback
    ...
    status = fallback.synthesis_fallback_runtime_status(max_age_seconds=1)
...
def test_missing_cursor_sdk_can_use_explicit_read_only_fallback(monkeypatch, tmp_path):
    from scripts.research_data_mcp import desk_brain, desk_synthesis_fallback
    ...
    monkeypatch.setattr(
        ...
        lambda: (_ for _ in ()).throw(desk_brain.CursorSdkUnavailable("missing sdk")),
    )
    monkeypatch.setattr(
        desk_synthesis_fallback,
        "run_gemini_synthesis_turn",
        lambda _prompt: (
            "A provisional proxy could use the supplied evidence. Which horizon matters most?",
            {
                "provider": "gemini",
                "model": "gemini-test",
                "read_only": True,
    ...
    assert turn.action_result["action"] == "synthesis_reasoning"
    assert turn.action_result["fallback_from"] == "cursor_sdk_unavailable"
    assert state["synthesis_thread_turns"]["thread-sdk-fallback"] == 1
